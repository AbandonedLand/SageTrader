...
class TibetSwap
{
    ...
    private static float $donation = 0.003;
    ...
    public static function syncTibetAssets(){
        $tibettokens = self::getPairs(0,1000);
        foreach ($tibettokens as $tibettoken) {
            $asset = \App\Models\Asset::where('asset_id', $tibettoken['asset_id'])->first();
            if ($asset) {
                $asset->tibetswap_pair_id = $tibettoken['launcher_id'];
                $asset->tibetswap_liquidity_asset_id = $tibettoken['liquidity_asset_id'];
                $asset->save();
            }
        }
    }
